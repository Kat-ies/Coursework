...
# function ds_create makes a new marked-up dataset
# the wider_face dataset is not suitable for the face classification task
# to solve the classification problem, we want to input an image of a face or not a face
...
# from the location diagonally from the face, i.e. the images have a common point
...
def ds_create(images_dict, frames_dict):
    faces = []
    for key in frames_dict:
        for rect in frames_dict[key]:
            if rect == (0, 0, 0, 0):
                img_not_face = images_dict[key].crop(random_box).resize(MAXSIZE, Image.LANCZOS)
